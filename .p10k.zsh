# Generated by Powerlevel10k configuration wizard on 2024-10-10 at 12:46 BST.# Wizard options: nerdfont-v3 + powerline, large icons, rainbow, unicode, 24h time,# darkest-ornaments, sparse, few icons, concise, transient_prompt,# Type `p10k configure` to generate another config.
   ...
   typeset -g POWERLEVEL9K_VCS_BRANCH_ICON=
    ...
    if [[ -n $P9K_CONTENT ]]; then
      ...
      return
       ...
       local       meta='%7F'    local      clean='%0F'    local   modified='%0F'    local  untracked='%0F'    local conflicted='%1F'
    local res

    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      local branch=${(V)VCS_STATUS_LOCAL_BRANCH}
                     (( $#branch > 32 )) && branch[13,-13]="…"       res+="${clean}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch//\%/%%}"
    fi

    if [[ -n $VCS_STATUS_TAG
                            && -z $VCS_STATUS_LOCAL_BRANCH         ]]; then
      local tag=${(V)VCS_STATUS_TAG}
                     (( $#tag > 32 )) && tag[13,-13]="…"       res+="${meta}#${clean}${tag//\%/%%}"
    fi
